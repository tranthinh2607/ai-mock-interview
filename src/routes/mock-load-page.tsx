...
export const MockLoadPage = () => {
  ...
  const hasRedirected = useRef(false); // Thêm cờ
  const navigate = useNavigate();

  // HÀM ĐƯỢC CẬP NHẬT: Lấy chi tiết phỏng vấn kèm kiểm tra trạng thái
  const getInterviewDetails = async () => {
    ...
    try {
      ...
      if (interviewSnap.exists()) {
        // Tạo đối tượng hoàn chỉnh trước khi kiểm tra
        const data = {
          id: interviewSnap.id,
          ...interviewSnap.data(),
        } as Interview;
  ...
  // Cập nhật useEffect để dùng hàm mới
  useEffect(() => {
    setIsLoading(true);
    getInterviewDetails();
  }, [interviewId]);  //[interviewId, navigate]);
